...
const StoryOfSoil = ({
  StoryOfSoilContainerRef,
}: {
  StoryOfSoilContainerRef: React.RefObject<HTMLDivElement | null>;
}) => {
  const [active, setActive] = useState<boolean>(true);
  const language = useStore((state) => state.language);

  const { scrollYProgress } = useScroll({
    target: StoryOfSoilContainerRef,
            ...
            <div>
              <Link href={seasons[0].href}>
                ...
                >
                  ...
                  >
                    <Image
                      src={`${process.env.NEXT_PUBLIC_UPLOADS_BASE_URL}${seasons[0].imageUrls[1]}`}
                      alt={seasons[0].slug}
                      fill
                      objectFit="cover"
                      className="mask-image"
